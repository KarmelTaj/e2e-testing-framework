...
# Partner To-do List Base Scenario
class ToDoListBaseScenario(BaseScenario):
    """
    Base class for To Do List scenarios
    """
    ...
    def generate_unique_title(self):
        # Generate a base title using the full timestamp
        self.set_step("Unique Title")
        base_title = f"[To Do List] {time.current_time()}"
        get_list_by_title = {"text": base_title}

        # Refine the title in case of duplicates
        suffix_counter = 0
        while True:
            list_by_title, status_code = self.call(
                ToDoListEndpoints.GET_TO_DO_LIST_BY_TITLE, params=get_list_by_title
            )
            Assert.assertEqual(status_code, 200)

            if list_by_title and list_by_title["count"] > 0:
                # Title exists, append a counter to make it unique
                self.warning(
                    f"Duplicate To Do List found with text: '{get_list_by_title['text']}'"
                )
                suffix_counter += 1
                get_list_by_title["text"] = f"{base_title} ({suffix_counter})"
            else:
                # Found a unique title
                break
    ...
    def get_series_items(self, title, item_counts):
        ...
        Assert.assertGreaterThan(to_do_list_series_items["count"], 0)
    ...
    def delete_series(self, slug):
        delete_series_endpoint = self.format_endpoint(
            ToDoListEndpoints.DELETE_TO_DO_LIST, slug=slug
        )
        _, status_code = self.call(delete_series_endpoint)
        Assert.assertEqual(status_code, 204)


# ---------------- Partner Scenarios ----------------
class ToDoListTestScenario(ToDoListBaseScenario):
    """
...
class ToDoListDailyTestScenario(ToDoListBaseScenario):
    """
...
class ToDoListWeeklyTestScenario(ToDoListBaseScenario):
    """
...
class ToDoListMonthlyTestScenario(ToDoListBaseScenario):
    """
...
class ToDoListYearlyTestScenario(ToDoListBaseScenario):
    """
...
class ToDoListUpdateTestScenario(ToDoListBaseScenario):
    """
...
class ToDoListMyTeamTestScenario(ToDoListBaseScenario):
    """
    ...
    def run(self):
        ...
        self.get_series_items_for_team(series["title"], 11, origin="my_team_tasks")
...
class ToDoListMeAndMyTeamTestScenario(ToDoListBaseScenario):
    """
...
class ToDoListBadDataTestScenario(ToDoListBaseScenario):
    """
    ...
    def run(self):
        ...
        for index, bad_data in enumerate(bad_data_cases, start=1):
            ...
            _, status_code = self.call(
                ToDoListEndpoints.CREATE_TO_DO_LIST, to_do_list_data)
            Assert.assertEqual(status_code, 400, f"Failed test for {
                               index}. Expected 400 but got {status_code}")
# ---------------- End of Partner Scenarios ----------------

# Backoffice Base Scenario
class ToDoListBaseBackOfficeScenario(BaseScenario):
    """
    Base class for To Do List BACKOFFICE  Scenarios 
    """
    ...
    def generate_unique_title(self):
        ...
        while True:
            list_by_title, status_code = self.call(
                ToDoListEndpoints.GET_TO_DO_LIST_BY_TITLE_BACKOFFICE, params=get_list_by_title
            )
            Assert.assertEqual(status_code, 200)
    ...
    def update_series(
        ...
        update_series_endpoint = self.format_endpoint(
            ToDoListEndpoints.UPDATE_TO_DO_LIST_BACKOFFICE, slug=slug
        )
        to_do_list_series, status_code = self.call(
            update_series_endpoint, to_do_list_data
        )
    ...
    def delete_series(self, slug):
        delete_series_endpoint = self.format_endpoint(
            ToDoListEndpoints.DELETE_TO_DO_LIST_BACKOFFICE, slug=slug
        )
        _, status_code = self.call(delete_series_endpoint)
        Assert.assertEqual(status_code, 204)

# ---------------- Backoffice Scenarios ----------------
class ToDoListBackOfficeCareerLevelTestScenario(ToDoListBaseBackOfficeScenario):
    """
    ...
    def run(self):
        ...
        todo_assignments = self.create_todo_assignments_list_data(career_levels=career_levels)
...
class ToDoListBackOfficePartnerTypeTestScenario(ToDoListBaseBackOfficeScenario):
    """
    ...
    def run(self):
        ...
        todo_assignments = self.create_todo_assignments_list_data(partner_types=partner_types)
        series = self.create_series(
            title=unique_title,
            frequency="weekly",
            frequency_coefficient="2",
...
class ToDoListBackOfficePartnerAndCareerLevelTestScenario(ToDoListBaseBackOfficeScenario):
    """
    Tets To-Do-List Backoffice Partner type field 
    """
    ...
    def run(self):
        ...
        career_levels = [career_level]
        partner_types = [partner_type]
        todo_assignments = self.create_todo_assignments_list_data(career_levels, partner_types)
        series = self.create_series(
            title=unique_title,
            frequency="weekly",
            frequency_coefficient="2",
...
class ToDoListBackOfficeMultipleCareerLevelTestScenario(ToDoListBaseBackOfficeScenario):
    """
    Tets To-Do-List Backoffice Partner type field 
    """
...
class ToDoListBackOfficeMultiplePartnerTestScenario(ToDoListBaseBackOfficeScenario):
    """
    Tets To-Do-List Backoffice Partner type field 
    """
...
class ToDoListBackOfficeMultiplePartnerAndCareerLevelTestScenario(ToDoListBaseBackOfficeScenario):
    """
    Tets To-Do-List Backoffice Partner type field 
    """
...
class ToDoListBackOfficeNotInTheListTestScenario(ToDoListBaseBackOfficeScenario):
    """
    Tets To-Do-List Backoffice Partner type field 
    """
